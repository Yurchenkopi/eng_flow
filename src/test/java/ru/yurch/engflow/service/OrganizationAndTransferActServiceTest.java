...
class OrganizationAndTransferActServiceTest{
    @Autowired OrganizationService organizations;@Autowired ContactService contacts;@Autowired ProjectRepository projects;@Autowired CatalogItemRepository catalogItems;@Autowired ProjectItemRepository projectItems;@Autowired ProjectAssemblyRepository assemblies;@Autowired TransferActService acts;
    ...
    @Test void newActDefaultsDateAndDestinationFromSection(){ProjectItem item=projectItem("ИТ809.00.00.000","Клапан",BigDecimal.ONE);ProjectAssembly assembly=new ProjectAssembly();assembly.setProject(item.getProject());assembly.setName("Вакуумная система");assembly.setDesignation("ИТ809.03");assembly=assemblies.save(assembly);allocation(item).setProjectAssembly(assembly);projectItems.saveAndFlush(item);TransferAct prepared=acts.prepare(item.getProject().getId(),List.of(allocation(item).getId()));assertThat(prepared.getActDate()).isEqualTo(LocalDate.now());assertThat(prepared.getItems().getFirst().getDestinationDesignation()).isEqualTo("ИТ809.03");allocation(item).setProjectAssembly(null);projectItems.saveAndFlush(item);assertThat(acts.prepare(item.getProject().getId(),List.of(allocation(item).getId())).getItems().getFirst().getDestinationDesignation()).isNull();}
