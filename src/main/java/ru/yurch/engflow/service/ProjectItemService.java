    ...
    private void resolveReferences(Long projectId, ProjectItem item) {
        ...
        for (ProjectItemAllocation allocation : item.getAllocations()) {
            ...
            if(allocation.isDetailForTransfer()){
                ...
                else allocation.setProjectSubsection(subsectionService.resolve(allocation.getProjectAssembly(),allocation.getSubsectionDesignation()));
                allocation.setAppliesFor(trimToNull(allocation.getSubsectionAppliesFor()));
            }else {allocation.setProjectSubsection(null);allocation.setAppliesFor(null);}
        ...
        for(ProjectItemAllocation allocation:item.getAllocations()){
            ...
            String key=assemblyId==null?"without-section":assemblyId+":"+(subsectionId==null?"without-subsection":subsectionId+":"+Objects.toString(allocation.getAppliesFor(),""));
            if(!allocationKeys.add(key))throw new IllegalArgumentException(subsectionId==null?"Обычная строка раздела может встречаться только один раз":"Комбинация подраздела и назначения уже указана");
        }
    }
