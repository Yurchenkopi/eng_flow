...
public class ItemSupplierService{
 ...
 public ItemSupplier find(Long catalogItemId,Long id){return repository.findById(id).filter(value->value.getCatalogItem().getId().equals(catalogItemId)).orElseThrow(()->new IllegalArgumentException("Поставщик изделия не найден"));}
 ...
 @Transactional public ItemSupplier update(Long catalogItemId,Long id,ItemSupplier values){ItemSupplier relation=find(catalogItemId,id);relation.setSupplierArticle(trim(values.getSupplierArticle()));relation.setNotes(trim(values.getNotes()));return repository.save(relation);}
