...
public interface CatalogItemRepository extends JpaRepository<CatalogItem, Long> {
    ...
    List<CatalogItem> findAll(Sort sort);
    ...
    @Query("""
            ...
            where lower(coalesce(item.designation, '')) like lower(concat('%', :query, '%'))
               ...
               or exists (select relation.id from ItemSupplier relation where relation.catalogItem=item \
            and (lower(relation.supplier.name) like lower(concat('%', :query, '%')) \
            or lower(coalesce(relation.supplier.shortName,'')) like lower(concat('%', :query, '%'))))
            """)
