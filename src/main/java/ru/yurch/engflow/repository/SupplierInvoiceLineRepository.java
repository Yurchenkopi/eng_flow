...
public interface SupplierInvoiceLineRepository extends JpaRepository<SupplierInvoiceLine,Long> {
    ...
    @Query("select coalesce(sum(line.quantity),0) from SupplierInvoiceLine line where line.supplierInvoice.procurement.id=:procurementId and line.supplierInvoice.paymentSubmittedDate is not null")
