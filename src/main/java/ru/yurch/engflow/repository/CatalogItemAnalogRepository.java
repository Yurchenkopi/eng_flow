...
public interface CatalogItemAnalogRepository extends JpaRepository<CatalogItemAnalog,Long> {
    @EntityGraph(attributePaths={"firstCatalogItem","firstCatalogItem.measurementUnit","secondCatalogItem","secondCatalogItem.measurementUnit"})
    @Query("select relation from CatalogItemAnalog relation where relation.firstCatalogItem.id=:itemId or relation.secondCatalogItem.id=:itemId order by relation.id")
