...
public interface ProjectItemRepository extends JpaRepository<ProjectItem, Long> {
    @EntityGraph(attributePaths = {"catalogItem", "catalogItem.measurementUnit", "allocations", "allocations.projectAssembly", "allocations.projectSubsection"})
    ...
    List<ProjectItem> search(@Param("projectId") Long projectId, @Param("query") String query,
                             @Param("assemblyId") Long assemblyId, @Param("supplierId") Long supplierId, Sort sort);
    ...
    @Query("select coalesce(sum(allocation.quantity),0) from ProjectItemAllocation allocation where allocation.projectItem.id = :projectItemId")
