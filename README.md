...
EngFlow — внутреннее веб-приложение для инженерной работы. Текущая версия позволяет вести организации и их контакты, проекты установок, общий каталог покупных изделий, разделы и комплектацию проекта, оформлять акты передачи позиций комплектации в цех, а также вести закупки проекта, запросы поставщикам и счета.
...
Пароль из примера замените локальным и не добавляйте его в Git. Таблицы автоматически создаются Liquibase при первом запуске. Hibernate работает в режиме `validate` и не создает схему самостоятельно.
...
| `INVOICE_STORAGE` | `./data/invoices` | Каталог файлов счетов поставщиков |
